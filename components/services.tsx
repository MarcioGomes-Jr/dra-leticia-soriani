...
import { Eye, Heart, Sparkles, Shield, Dna, ArrowUpRight } from "lucide-react"
...
const services = [
  ...
  {
    icon: Dna,
    title: "Epigenética e Nutrigenética",
    description:
      "Análise individualizada das tendências genéticas e condições oftalmológicas já existentes, visando prevenção, equilíbrio sistêmico e otimização da função visual.",
  },
]
...
export function Services() {
  ...
  return (
    <section id="servicos" className="py-20 md:py-32 bg-background relative overflow-hidden">
      <div className="absolute top-20 right-20 w-72 h-72 bg-[#D8C17A]/12 rounded-full blur-3xl animate-pulse" />
      <div className="absolute bottom-20 left-20 w-96 h-96 bg-[#7A7566]/8 rounded-full blur-3xl" />
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        ...
        >
          ...
          <h2 className="font-serif text-3xl md:text-4xl lg:text-5xl font-medium text-[#7A7566] mb-6 leading-tight text-balance">
            De ponta a ponta, desde o <span className="text-[#A98E3F]">diagnóstico</span> até o tratamento
          </h2>
          ...
          {services.map((service, index) => (
            ...
            >
              <div className="w-12 h-12 bg-[#7A7566]/10 rounded-full flex items-center justify-center mb-5 group-hover:bg-[#7A7566] transition-colors">
                <service.icon className="w-6 h-6 text-[#7A7566] group-hover:text-[#D8C17A] transition-colors" />
              </div>
              ...
              <a
                ...
                className="mt-5 inline-flex items-center gap-2 text-[#A98E3F] font-medium text-sm group/link"
              >
                Saiba mais
        ...
        <div className={`text-center mt-20 p-8 md:p-12 bg-gradient-to-r from-[#7A7566]/10 via-[#D8C17A]/12 to-[#7A7566]/10 rounded-2xl border border-[#D8C17A]/40 transition-all duration-700 delay-300 ${
          isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
        }`}>
          <h3 className="font-serif text-2xl md:text-3xl font-medium text-[#7A7566] mb-4">
            Não deixe <span className="text-[#A98E3F]">para depois</span>
          </h3>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="group inline-flex items-center gap-2 px-8 py-4 bg-[#7A7566] text-white font-medium rounded-full hover:bg-[#7A7566] transition-all duration-300 hover:shadow-xl hover:scale-105"
          >
